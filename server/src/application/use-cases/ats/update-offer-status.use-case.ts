...
export class UpdateOfferStatusUseCase implements IUpdateOfferStatusUseCase {
  ...
  async execute(data: {
    ...
    // Get offer first to get applicationId
    ...
    if (data.status === 'sent') {
      ...
      // If withdrawalReason is provided, this is a company withdrawal (not candidate decline)
    ...
    // Update application substage to OFFER_DECLINED when company withdraws
    ...
    // Log activity if status changed to signed or declined
