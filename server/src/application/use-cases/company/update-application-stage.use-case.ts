...
export class UpdateApplicationStageUseCase implements IUpdateApplicationStageUseCase {
  ...
  async execute(userId: string, applicationId: string, stage: 'applied' | 'shortlisted' | 'interview' | 'rejected' | 'hired', rejectionReason?: string): Promise<JobApplicationListResponseDto> {
