...
export interface IPaymentOrderRepository extends IBaseRepository<PaymentOrder> {
  ...
  sumEarningsByDateRange(startDate: Date, endDate: Date): Promise<number>;
  getMonthlyEarnings(year: number): Promise<{ month: number; amount: number }[]>;
  getEarningsByPeriod(period: 'all' | 'day' | 'week' | 'month' | 'year', startDate?: Date, endDate?: Date): Promise<{ label: string; value: number }[]>;
