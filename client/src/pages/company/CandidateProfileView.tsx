...
import { Loader2, ArrowLeft, Mail, Phone, MessageSquare, MessageCircle, ChevronRight, CheckCircle2, Circle, Clock, Calendar, Video, Star, Plus, X, Globe, ExternalLink, Briefcase, User, Download, MapPin, Instagram, Twitter, File as FileIcon, Edit, XCircle, DollarSign, TrendingUp, CheckCircle, FileText, Bell, UserCheck, UserX, AlertTriangle } from 'lucide-react'
...
import type { ATSInterview, ATSComment, ATSTechnicalTask, ATSOfferDocument } from '@/types/ats'
...
import { ATSStage, STAGE_SUB_STAGES, ATSStageDisplayNames, ShortlistedSubStage, InterviewSubStage, TechnicalTaskSubStage, CompensationSubStage, OfferSubStage, SubStageDisplayNames } from '@/constants/ats-stages'
...
const CandidateProfileView = () => {
    ...
    // Non-ATS Data
    const [candidateData, setCandidateData] = useState<CandidateDetailsResponse | null>(null)

    // ATS Data
    const [atsApplication, setAtsApplication] = useState<CompanySideApplication | null>(null)
    ...
    const [offerDocuments, setOfferDocuments] = useState<ExtendedATSOfferDocument[]>([])
    const [comments, setComments] = useState<ATSComment[]>([])

    const [loading, setLoading] = useState(true)
    const [activeTab, setActiveTab] = useState<'profile' | 'resume' | 'hiring' | 'activity'>('profile')
    ...
    const [selectedInterviewForFeedback, setSelectedInterviewForFeedback] = useState<ATSInterview | null>(null)
    const [selectedInterviewForReschedule, setSelectedInterviewForReschedule] = useState<ATSInterview | null>(null)
    const [showAssignTaskModal, setShowAssignTaskModal] = useState(false)
    const [selectedTaskForReview, setSelectedTaskForReview] = useState<ExtendedATSTechnicalTask | null>(null)
    const [selectedTaskForEdit, setSelectedTaskForEdit] = useState<ExtendedATSTechnicalTask | null>(null)
    ...
    const [compensationNotes, setCompensationNotes] = useState<Array<{
        comment: string;
    ...
    const [selectedMeetingForEdit, setSelectedMeetingForEdit] = useState<CompensationMeeting | null>(null)
    
    // Offer state
    const [currentOffer, setCurrentOffer] = useState<ExtendedATSOfferDocument | null>(null)
    ...
    const isATSMode = location.pathname.includes('/ats/candidate/') || location.pathname.includes('/company/applicants/')
    const currentId = candidateId || id

    const getInitials = (name: string) => {
        return name?.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'CN'
    }
        ...
        try {
            if (isATSMode) {
                ...
                if (appRes.data) {
                    ...
                    const getNumber = (val: unknown): number | undefined => typeof val === 'number' ? val : undefined
                    const getStringArray = (val: unknown): string[] | undefined => Array.isArray(val) && val.every(v => typeof v === 'string') ? val as string[] : undefined
                    const mappedApplication: CompanySideApplication = {
                        ...
                        date_of_birth: getString(applicationData.date_of_birth) || (applicationData.date_of_birth as Date | undefined),
                        ...
                        resume_data: applicationData.resume_data as CompanySideApplication['resume_data'],
                    ...
                    const jobId = getString(applicationData.job_id) || mappedApplication.jobId || mappedApplication.job?.id
                    const seekerId = getString(applicationData.seeker_id) || mappedApplication.seekerId || mappedApplication.seeker?.id
                    const currentStage = mappedApplication.stage

                    // Fetch Job Details, Candidate Details, and ATS Activities in parallel
                    const fetchPromises = [
                        // Job Details
                        jobId ? companyApi.getJobPosting(jobId).catch((e) => {
                            console.error("Failed to fetch job", e)
                            return { data: null }
                        }) : Promise.resolve({ data: null }),
                        // Candidate Details - use server data if available, otherwise fetch
                        seekerId ? companyApi.getCandidateDetails(seekerId).catch((e) => {
                            console.error("Failed to fetch candidate details", e)
                            return { data: null }
                        }) : Promise.resolve({ data: null }),
                        // ATS Activities
                        atsService.getInterviewsByApplication(currentId).catch(() => ({ data: [] })),
                        atsService.getTechnicalTasksByApplication(currentId).catch(() => ({ data: [] })),
                        atsService.getOffersByApplication(currentId).catch(() => ({ data: [] })),
                        atsService.getCommentsByApplication(currentId).catch(() => ({ data: [] })),
                        // Activity Logs - use paginated API
                        atsService.getActivitiesByApplicationPaginated(currentId, 20).catch(() => ({ activities: [], nextCursor: null, hasMore: false }))
                    ]
                    ...
                    const [jobRes, candRes, interviewsRes, tasksRes, offersRes, commentsRes, activitiesRes, ...compensationResults] = results
                    ...
                    } else if (getString(applicationData.seeker_name) || getString(applicationData.email)) {
                        // If candidate details fetch failed but we have data in application, create minimal candidate data
                        const resumeData = applicationData.resume_data as { experience?: Array<Record<string, unknown>>; education?: Array<Record<string, unknown>> } | undefined
                        setCandidateData({
                            profile: {
                                name: getString(applicationData.seeker_name || applicationData.full_name) || '',
                                email: getString(applicationData.email) || '',
                            ...
                            experiences: resumeData?.experience?.map((exp: Record<string, unknown>) => {
                            ...
                            educations: resumeData?.education?.map((edu: Record<string, unknown>) => {
                    ...
                    if (applicationStage === ATSStage.OFFER && offersRes.data && offersRes.data.length > 0) {
                    ...
                    const activitiesData = activitiesRes || { activities: [], nextCursor: null, hasMore: false }
                    ...
                    if (currentStage === ATSStage.COMPENSATION && compensationResults.length >= 3) {
                        ...
                        const meetingsData: CompensationMeeting[] = Array.isArray(meetingsRes) 
                            ? meetingsRes.filter((m): m is CompensationMeeting => m != null).map(m => ({
                                id: typeof m === 'object' && m !== null && 'id' in m ? String(m.id) : undefined,
                                type: (typeof m === 'object' && m !== null && 'type' in m && (m.type === 'call' || m.type === 'online' || m.type === 'in-person')) 
                                    ? m.type 
                                    : 'call',
                                scheduledDate: typeof m === 'object' && m !== null && 'scheduledDate' in m ? String(m.scheduledDate) : '',
                                location: typeof m === 'object' && m !== null && 'location' in m ? String(m.location) : undefined,
                                meetingLink: typeof m === 'object' && m !== null && 'meetingLink' in m ? String(m.meetingLink) : undefined,
                                notes: typeof m === 'object' && m !== null && 'notes' in m ? String(m.notes) : undefined,
                                status: typeof m === 'object' && m !== null && 'status' in m && (m.status === 'scheduled' || m.status === 'completed' || m.status === 'cancelled')
                                    ? m.status
                                    : undefined,
                                completedAt: typeof m === 'object' && m !== null && 'completedAt' in m ? String(m.completedAt) : undefined,
                                createdAt: typeof m === 'object' && m !== null && 'createdAt' in m ? String(m.createdAt) : new Date().toISOString(),
                                updatedAt: typeof m === 'object' && m !== null && 'updatedAt' in m ? String(m.updatedAt) : undefined
                            }))
                            : []
            ...
            toast({ title: "Error", description: "Failed to load candidate data.", variant: "destructive" })
        }
    }, [currentId, isATSMode])
    ...
    const hiringStages = useMemo(() => {
        const allStages = (atsJob?.enabled_stages as string[]) || Object.values(ATSStage)
        ...
        return stagesWithApplied.map((stage, index) => {
            ...
            const isCompleted = isApplied ? isAppliedCompleted : index < finalCurrentStageIndex
    ...
    const handleAddComment = async (comment: string) => {
        ...
        try {
            ...
            const validStage = (selectedStage === 'APPLIED' || selectedStage === 'Applied') 
    ...
    const handleScheduleInterview = async (data: { title: string; type: 'online' | 'offline'; date: string; time: string; location?: string; meetingLink?: string; notes?: string }, interviewId?: string) => {
        if (!currentId) return
        ...
        try {
            ...
            if (interviewId) {
                ...
                if (atsApplication?.stage !== ATSStage.INTERVIEW) {
                    ...
                    if (atsApplication.subStage !== InterviewSubStage.SCHEDULED && 
            ...
            toast({ title: "Error", description: interviewId ? "Failed to reschedule interview." : "Failed to schedule interview.", variant: "destructive" })
        }
    }
    ...
    const handleAssignTask = async (taskData: { title: string; description: string; deadline: string; documentUrl?: string; documentFilename?: string; document?: File }) => {
        if (!currentId) return
        try {
            if (selectedTaskForEdit?.id) {
                ...
                if (atsApplication?.stage === ATSStage.TECHNICAL_TASK) {
                    await companyApi.moveApplicationStage(currentId, {
                        ...
                        comment: `Technical task assigned – Due ${new Date(taskData.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`
                    })
                }
            ...
            toast({ title: "Error", description: selectedTaskForEdit ? "Failed to update task." : "Failed to assign task.", variant: "destructive" })
        }
    }
    ...
    const handleInitiateCompensation = async (data: { candidateExpected: string; companyProposed?: string; benefits?: string[]; expectedJoining?: string; notes?: string }) => {
        if (!currentId) return
        try {
            ...
            toast({ title: "Success", description: existingCompensation ? "Expected salary updated successfully." : "Compensation discussion started successfully." })
        } catch (error) {
            console.error("Failed to initiate/update compensation:", error)
            toast({ title: "Error", description: "Failed to save expected salary.", variant: "destructive" })
        }
    }

    const handleUpdateCompensation = async (data: { companyProposed?: string; expectedJoining?: string; benefits?: string[]; notes?: string }) => {
        if (!currentId) return
        try {
            ...
            const isSubsequentUpdate = compensationData?.companyProposed || 
                                     (compensationData?.updatedAt && compensationData?.createdAt && 
    ...
    const handleScheduleCompensationMeeting = async (data: { type: string; date: string; time: string; location?: string; meetingLink?: string; notes?: string }) => {
        if (!currentId) return
        try {
            const meetingRes = await atsService.scheduleCompensationMeeting(currentId, data)
            // Update local state immediately
            setCompensationMeetings(prev => [...(prev || []), meetingRes.data])
            toast({ title: "Success", description: "Compensation meeting scheduled successfully." })
        } catch (error) {
            console.error("Failed to schedule compensation meeting:", error)
            toast({ title: "Error", description: "Failed to schedule compensation meeting.", variant: "destructive" })
        }
    }

    // Offer handlers
    const handleCreateOffer = async (data: { offerAmount: string; employmentType: string; joiningDate: string; validityDate: string; documentUrl?: string; documentFilename?: string; document?: File; notes?: string }) => {
        if (!currentId) return
        try {
            ...
            toast({ title: "Success", description: "Offer created and sent successfully." })
            await reloadData()
        } catch (error) {
    ...
    const handleCloseApplication = async () => {
        if (!currentId) return
        try {
            // Move to REJECTED stage or close application
            await companyApi.moveApplicationStage(currentId, {
                nextStage: 'REJECTED' as ATSStage,
                comment: 'Application closed - Offer declined'
            })
            toast({ title: "Success", description: "Application closed." })
            await reloadData()
        } catch (error) {
    ...
    const interviewSummary = useMemo(() => {
        ...
        const allCompletedHaveFeedback = completedInterviews.length > 0 && 
    ...
    const renderInReviewStage = () => {
        ...
        return (
            <div className="bg-gray-50 rounded-lg p-6 space-y-6">
                <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">In Review Stage</h3>
                    {showActions && (
                    <p className="text-sm text-gray-600 mb-4">
                        Current sub-stage: <span className="font-medium text-gray-900">
                            {subStages.find(s => s.key === currentSubStage)?.label || currentSubStage}
                        </span>
                    </p>
                    )}
                </div>
                ...
                {comments.filter(c => c.stage === ATSStage.IN_REVIEW).length > 0 && (
                    <div className="space-y-3">
                        ...
                        {comments.filter(c => c.stage === ATSStage.IN_REVIEW).map((comment, idx) => (
                            <div key={idx} className="bg-white rounded-lg p-4 border">
                                <p className="text-sm text-gray-700">{comment.comment}</p>
                                <p className="text-xs text-gray-500 mt-2">
                                    by {comment.recruiterName || comment.addedByName || comment.addedBy || 'Recruiter'} • {formatDateTime(comment.createdAt || comment.timestamp || '')}
                                </p>
                            </div>
                        ))}
                    </div>
                ...
                {showActions && (
                    ...
                    {subStages.map((subStage, idx) => {
                        ...
                        if (currentSubStage === 'PENDING_DECISION' && subStage.key === 'PROFILE_REVIEW') return null
                        ...
                        return (
                            <Button
                                ...
                                className={`gap-2 ${idx === subStages.length - 1 ? 'bg-[#4640DE] hover:bg-[#3730A3]' : ''}`}
                                onClick={async () => {
                                    if (!currentId) return
                                    try {
                                        ...
                                        toast({ title: "Success", description: `Moved to ${subStage.label}` })
                                        await reloadData()
                                    } catch (error: unknown) {
                                        console.error("Failed to update sub-stage:", error)
                                        toast({ 
                                            title: "Error", 
                                            description: (error as { response?: { data?: { message?: string } } })?.response?.data?.message || "Failed to update sub-stage.", 
                                            variant: "destructive" 
                                        })
                                    }
                                }}
                    ...
                    {currentSubStage === 'PENDING_DECISION' && (
                        <div className="flex gap-3 pt-2 border-t">
                            <Button
                                ...
                                onClick={() => handleUpdateStage(ATSStage.SHORTLISTED, 'READY_FOR_INTERVIEW')}
                            >
                                <ChevronRight className="h-4 w-4" />
                                Move to Shortlisted
                            </Button>
                            {/* Hide reject button when stage is OFFER and substage is OFFER_SENT */}
                            {!(selectedStage === ATSStage.OFFER && String(currentSubStage) === String(OfferSubStage.OFFER_SENT)) && (
                                <Button
                                    variant="destructive"
    ...
    const renderShortlistedStage = () => {
        ...
        return (
            <div className="bg-gray-50 rounded-lg p-6 space-y-6">
                <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Shortlisted Stage</h3>
                    {showActions && (
                    <Badge className="bg-blue-100 text-blue-700 mb-4">
                        Current Sub-stage: {currentSubStage === 'READY_FOR_INTERVIEW' ? 'Ready for Interview' :
                            currentSubStage === 'CONTACTED' ? 'Contacted' :
                                currentSubStage === 'AWAITING_RESPONSE' ? 'Awaiting Response' : currentSubStage}
                    </Badge>
                    )}
                </div>
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        toast({ title: "Success", description: "Marked as Contacted" })
                                        await reloadData()
                                    } catch (error: unknown) {
                                        console.error("Failed to update sub-stage:", error)
                                        toast({ 
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        toast({ title: "Success", description: "Marked as Awaiting Response" })
                                        await reloadData()
                                    } catch (error: unknown) {
                                        console.error("Failed to update sub-stage:", error)
                                        toast({ 
                ...
                {comments.filter(c => c.stage === ATSStage.SHORTLISTED).length > 0 && (
                    <div className="space-y-3 pt-4 border-t">
                        <h4 className="font-medium text-gray-900">Comments</h4>
                        {comments.filter(c => c.stage === ATSStage.SHORTLISTED).map((comment, idx) => (
                            <div key={idx} className="bg-white rounded-lg p-4 border">
                                <p className="text-sm text-gray-700">{comment.comment}</p>
                                <p className="text-xs text-gray-500 mt-2">
                                    by {comment.recruiterName || comment.addedByName || comment.addedBy || 'Recruiter'} • {formatDateTime(comment.createdAt || comment.timestamp || '')}
    ...
    const renderInterviewStage = () => {
        const { overallStatus, interviewsConducted, upcomingInterview } = interviewSummary
        const currentSubStage = atsApplication?.subStage || InterviewSubStage.NOT_SCHEDULED
        // Only show action buttons if this is the current stage
        const showActions = isCurrentStage(selectedStage)
        ...
        return (
            <div className="bg-gray-50 rounded-lg p-6 space-y-6">
                ...
                <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Interview Stage</h3>
                    {showActions && (
                    <Badge className="bg-purple-100 text-purple-700 mb-4">
                            Current Sub-stage: {currentSubStage === InterviewSubStage.NOT_SCHEDULED ? 'Not Scheduled' :
                                currentSubStage === InterviewSubStage.SCHEDULED ? 'Scheduled' :
                            currentSubStage === InterviewSubStage.COMPLETED ? 'Completed' :
                            currentSubStage === InterviewSubStage.EVALUATION_PENDING ? 'Evaluation Pending' :
                            currentSubStage}
                    </Badge>
                            )}
                        </div>
                ...
                <div className="bg-white rounded-lg p-4 border">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <Badge className={
                            overallStatus === 'Ready for Decision' ? 'bg-green-100 text-green-700' :
                                overallStatus === 'Evaluation Pending' ? 'bg-yellow-100 text-yellow-700' :
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">{interviewsConducted}</p>
                        </div>
                        {upcomingInterview && (
                            <div>
                                <p className="text-sm text-gray-600">Upcoming Interview</p>
                                <p className="text-sm font-medium text-gray-900">{upcomingInterview.title}</p>
                                <p className="text-xs text-gray-500">{formatDateTime(upcomingInterview.scheduledDate)}</p>
                            </div>
                        )}
                    </div>
                </div>
                                ...
                                <div key={idx} className="bg-white rounded-lg p-4 border">
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${interview.status === 'completed' ? 'bg-green-100' : 'bg-blue-100'
                                                }`}>
                                    ...
                                    <div className="pl-13 space-y-2 text-sm">
                                        ...
                                        {interview.meetingLink && interview.status === 'scheduled' && (
                                            <a href={interview.meetingLink} target="_blank" rel="noopener noreferrer" className="text-[#4640DE] hover:underline flex items-center gap-2">
                                                ...
                                                {interview.rating && (
                                                    <div className="flex items-center gap-1 mt-2">
                                                        {[1, 2, 3, 4, 5].map((star) => (
                                                            <Star
                                                                key={star}
                                                                className={`h-4 w-4 ${interview.rating && star <= interview.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                                                            />
                                                        ))}
                                                        <span className="text-sm text-gray-700 ml-2">{interview.rating}/5</span>
                                                    </div>
                                                )}
                                            </div>
                                        )}
                ...
                {showActions && (
                    <div className="flex flex-col gap-3">
                        ...
                        {currentSubStage === InterviewSubStage.EVALUATION_PENDING && (() => {
                            const nextStage = atsApplication?.stage ? getNextStage(atsApplication.stage) : null
                        ...
                        {atsApplication?.stage && hasNextStages(atsApplication.stage as ATSStage) && (
                        <Button
                            variant="outline"
                                className="gap-2 border-[#4640DE] text-[#4640DE] hover:bg-[#4640DE] hover:text-white"
                                onClick={() => setShowMoveToStageModal(true)}
                ...
                {/* Show comments */}
                {showActions && comments.filter(c => c.stage === ATSStage.INTERVIEW).length > 0 && (
                    <div className="space-y-3 pt-4 border-t">
                        <h4 className="font-medium text-gray-900">Comments</h4>
                        {comments.filter(c => c.stage === ATSStage.INTERVIEW).map((comment, idx) => (
                            <div key={idx} className="bg-white rounded-lg p-4 border">
                                <p className="text-sm text-gray-700">{comment.comment}</p>
                                <p className="text-xs text-gray-500 mt-2">
                                    by {comment.recruiterName || comment.addedByName || comment.addedBy || 'Recruiter'} • {formatDateTime(comment.createdAt || comment.timestamp || '')}
    ...
    const technicalTaskSummary = useMemo(() => {
        const currentSubStage = atsApplication?.subStage || TechnicalTaskSubStage.NOT_ASSIGNED
        // Filter out cancelled tasks when calculating summary
        const activeTasks = technicalTasks.filter((t: ExtendedATSTechnicalTask) => t.status !== 'completed')
        const assignedTasks = activeTasks.filter((t: ExtendedATSTechnicalTask) => t.status === 'assigned')
        const submittedTasks = activeTasks.filter((t: ExtendedATSTechnicalTask) => t.status === 'submitted')
        const underReviewTasks = activeTasks.filter((t: ExtendedATSTechnicalTask) => t.status === 'under_review')
        const completedTasks = activeTasks.filter((t: ExtendedATSTechnicalTask) => t.status === 'completed')
        ...
        } else if (completedTasks.length === activeTasks.length && activeTasks.length > 0) {
            // Only if ALL tasks are completed, substage is COMPLETED
            calculatedSubStage = TechnicalTaskSubStage.COMPLETED
        } else if (submittedTasks.length === activeTasks.length && submittedTasks.length > 0 && completedTasks.length === 0) {
    ...
    const renderTechnicalTaskStage = () => {
        ...
        return (
            <div className="bg-gray-50 rounded-lg p-6 space-y-6">
                {/* Header */}
                <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Technical Task Stage</h3>
                    {showActions && (
                        <Badge className="bg-purple-100 text-purple-700 mb-4">
                                ...
                                {technicalTasks.map((task: ExtendedATSTechnicalTask) => {
                                    ...
                                    return (
                                            ...
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center gap-4 text-sm text-gray-600">
                                                        <span>Due: {task.deadline ? formatDateTime(task.deadline) : 'No deadline'}</span>
                                                        <Badge className={
                                        ...
                                        {task.documentUrl && task.documentFilename && (
                                            ...
                                            {(isSubmitted || isUnderReview || isCompleted) && task.submissionUrl && (
                                            ...
                                            {isCompleted && (task.feedback || task.rating) && (
                                                <div className="mb-3 p-3 bg-green-50 rounded border">
                                                    ...
                                                    {task.feedback && (
                                                        <p className="text-sm text-gray-700 whitespace-pre-wrap">{task.feedback}</p>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {isCancelled && (
                                                <div className="mt-4 pt-4 border-t">
                                                    <p className="text-sm text-gray-500 italic">Task has been revoked</p>
                                                </div>
                                            )}
                                        </div>
                                    )
                ...
                {showActions && (
                    <div className="flex flex-col gap-3">
                        <Button variant="outline" className="gap-2" onClick={() => setShowCommentModal(true)}>
                            <MessageSquare className="h-4 w-4" />
                        Add Comment
                    </Button>
                    ...
                    {(() => {
                            ...
                            if (!nextStage) return null
                            const nextStageDisplayName = ATSStageDisplayNames[nextStage] || nextStage
                            const isCompleted = currentSubStage === TechnicalTaskSubStage.COMPLETED || currentSubStage === 'COMPLETED'
                            return (
                                <Button
                                    variant="outline"
                                    className="gap-2 border-[#4640DE] text-[#4640DE] hover:bg-[#4640DE] hover:text-white"
                ...
                {/* Show comments */}
                {showActions && comments.filter(c => c.stage === ATSStage.TECHNICAL_TASK).length > 0 && (
                    <div className="space-y-3 pt-4 border-t">
                        <h4 className="font-medium text-gray-900">Comments</h4>
                        {comments.filter(c => c.stage === ATSStage.TECHNICAL_TASK).map((comment, idx) => (
                            <div key={idx} className="bg-white rounded-lg p-4 border">
                                <p className="text-sm text-gray-700">{comment.comment}</p>
                                <p className="text-xs text-gray-500 mt-2">
                                    by {comment.recruiterName || comment.addedByName || comment.addedBy || 'Recruiter'} • {formatDateTime(comment.createdAt || comment.timestamp || '')}
    ...
    const renderCompensationStage = () => {
        ...
        const jobSalaryRange = jobSalaryMin > 0 && jobSalaryMax > 0 
        ...
        const hasNotesOrUpdates = compensationNotes.length > 0 || !!companyProposed || (compensationData?.updatedAt && compensationData?.createdAt && new Date(compensationData.updatedAt) > new Date(compensationData.createdAt))
        ...
        return (
            <div className="bg-gray-50 rounded-lg p-6 space-y-6">
                ...
                <div>
                    <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Compensation Stage</h3>
                {showActions && (
                        ...
                        <div className="flex items-center gap-2 mb-4">
                            {candidateEmail && (
                                <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => window.location.href = `mailto:${candidateEmail}`}
                                    className="gap-2"
                                >
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        // Create conversation
                                        const conversation = await chatApi.createConversation({ participantId: candidateData.user._id })
                                        const conversationId = (conversation as { _id?: string; id?: string })._id || (conversation as { _id?: string; id?: string }).id || ''
                                        if (conversationId) {
                                            navigate(`/company/chat/${conversationId}`)
                                        } else {
                                            toast({ title: "Error", description: "Failed to get conversation ID.", variant: "destructive" })
                                        }
                                    } catch (error) {
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium text-gray-900">
                                {currentSubStage === CompensationSubStage.NOT_INITIATED ? 'Not Initiated' :
                                 currentSubStage === CompensationSubStage.INITIATED ? 'Initiated' :
                                 currentSubStage === CompensationSubStage.NEGOTIATION_ONGOING ? 'Negotiation Ongoing' :
                                 'Approved'}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-600">Company Budget Range</p>
                            <p className="text-sm font-medium text-gray-900">{jobSalaryRange}</p>
                        </div>
                            ...
                            <>
                                {finalAgreed ? (
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-green-700">{finalAgreed}</p>
                                    </div>
                                ) : companyProposed ? (
                                    <div>
                                        <p className="text-sm text-gray-600">Proposed Range</p>
                                        <p className="text-sm font-medium text-amber-700">{companyProposed}</p>
                                    </div>
                                ) : null}
                                {expectedJoining && (
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-gray-900">
                                            {new Date(expectedJoining).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                        </p>
                                    </div>
                                )}
                            </>
                ...
                <div className="bg-white rounded-lg p-4 border">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <p className="text-sm text-gray-600">Candidate Expected Salary:</p>
                            <p className="text-sm font-medium text-gray-900">
                                {candidateExpected || '—'}
                            </p>
                        ...
                        {showActions && currentSubStage === CompensationSubStage.NOT_INITIATED && (
                            <Button
                                size="sm"
                                variant="default"
                ...
                {currentSubStage === CompensationSubStage.INITIATED && (
                    <div className="bg-white rounded-lg p-6 border space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <p className="text-sm text-gray-600 mb-1">Candidate Expected Salary</p>
                                <p className="text-sm font-medium text-gray-900">{candidateExpected || 'Not specified'}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-600 mb-1">Company Budget Range</p>
                                <p className="text-sm font-medium text-gray-900">{jobSalaryRange}</p>
                            </div>
                        </div>
                        {showActions && (
                    ...
                    <div className="bg-white rounded-lg p-6 border space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p className="text-sm font-medium text-gray-900">{companyProposed || 'Not set'}</p>
                            </div>
                            <div>
                        ...
                        {showActions && (
                            <div className="flex gap-2 pt-4 border-t">
                                    ...
                                    <Button
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                toast({ title: "Success", description: "Compensation approved successfully." })
                                            } catch (error) {
                                                console.error("Failed to approve compensation:", error)
                                                toast({ title: "Error", description: "Failed to approve compensation.", variant: "destructive" })
                                            }
                                        }}
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        setShowRevokeConfirmDialog(true)
                                        // We'll use a different confirmation for rejection
                                    }}
                ...
                {currentSubStage === CompensationSubStage.APPROVED && (
                    <div className="bg-white rounded-lg p-6 border space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <p className="text-sm text-gray-600 mb-1">Final Agreed Salary</p>
                                <p className="text-sm font-medium text-green-700">{finalAgreed || companyProposed || 'Not set'}</p>
                            </div>
                            {expectedJoining && (
                                <div>
                        ...
                        {approvedAt && (
                            <div className="pt-4 border-t">
                                <p className="text-xs text-gray-500">
                                    Approved on {formatDateTime(approvedAt)} by {approvedBy || 'Recruiter'}
                                </p>
                            </div>
                        )}
                        {showActions && (
                ...
                {currentSubStage !== CompensationSubStage.NOT_INITIATED && (
                    <div className="bg-white rounded-lg p-6 border">
                        ...
                        {compensationMeetings.length > 0 ? (
                            <div className="space-y-3">
                                {compensationMeetings.filter(meeting => meeting != null).map((meeting: CompensationMeeting, idx: number) => {
                                    const meetingType = meeting?.type || 'call'
                                    const isCall = meetingType === 'call'
                                    const isCompleted = meeting.status === 'completed'
                                    const isCancelled = meeting.status === 'cancelled'
                                    return (
                                        <div key={idx} className={`border rounded-lg p-4 ${isCompleted ? 'bg-green-50 border-green-200' : isCancelled ? 'bg-gray-50 border-gray-200' : ''}`}>
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    {isCompleted && meeting.completedAt && (
                                                        <p className="text-xs text-green-600 mt-1">
                                                            Completed on {formatDateTime(meeting.completedAt)}
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            {meeting.notes && (
                                                <div className="mt-3 pt-3 border-t">
                                                    <p className="text-xs font-medium text-gray-600 mb-1">Notes / Outcome:</p>
                                                    <p className="text-sm text-gray-700 whitespace-pre-wrap">{meeting.notes}</p>
                                                </div>
                                            )}
                                            {showActions && currentSubStage !== CompensationSubStage.APPROVED && !isCompleted && !isCancelled && (
                                                <div className="flex gap-2 mt-3 pt-3 border-t">
                                                    {isCall && candidatePhone && candidatePhone !== '-' && (
                                                        <Button
                                                            size="sm"
                                                            variant="outline"
                                                            onClick={() => window.location.href = `tel:${candidatePhone}`}
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={async () => {
                                                            ...
                                                            try {
                                                                await atsService.updateCompensationMeetingStatus(currentId, meeting.id, 'completed')
                                                                // Refresh meetings
                                                                const meetingsRes = await atsService.getCompensationMeetings(currentId)
                                                                const meetingsData = Array.isArray(meetingsRes.data) ? meetingsRes.data.filter((m: CompensationMeeting | null) => m != null) : (Array.isArray(meetingsRes) ? meetingsRes.filter((m: CompensationMeeting | null) => m != null) : [])
                                                                setCompensationMeetings(meetingsData)
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={async () => {
                                                            if (!currentId || !meeting.id) return
                                                            try {
                                                                await atsService.updateCompensationMeetingStatus(currentId, meeting.id, 'cancelled')
                                                                // Refresh meetings
                                                                const meetingsRes = await atsService.getCompensationMeetings(currentId)
                                                                const meetingsData = Array.isArray(meetingsRes.data) ? meetingsRes.data.filter((m: CompensationMeeting | null) => m != null) : (Array.isArray(meetingsRes) ? meetingsRes.filter((m: CompensationMeeting | null) => m != null) : [])
                                                                setCompensationMeetings(meetingsData)
                            ...
                            <p className="text-sm text-gray-500 italic">No meetings scheduled yet</p>
                        )}
                    </div>
                )}
                ...
                <div className="bg-white rounded-lg p-6 border">
                    ...
                    {compensationNotes.length > 0 || comments.filter(c => c.stage === ATSStage.COMPENSATION).length > 0 ? (
                        <div className="space-y-3">
                            {compensationNotes.map((note: { comment?: string; note?: string; recruiterName?: string; createdAt: string }, idx: number) => (
                                <div key={idx} className="border rounded-lg p-4">
                                    <p className="text-sm text-gray-700">{note.comment || note.note || ''}</p>
                                    <p className="text-xs text-gray-500 mt-2">
                                        by {note.recruiterName || 'Recruiter'} • {formatDateTime(note.createdAt || '')}
                                    </p>
                                </div>
                            ))}
                            {comments.filter(c => c.stage === ATSStage.COMPENSATION).map((comment, idx) => (
                                <div key={idx} className="border rounded-lg p-4">
                                    <p className="text-sm text-gray-700">{comment.comment}</p>
                                    <p className="text-xs text-gray-500 mt-2">
                                        by {comment.recruiterName || comment.addedByName || comment.addedBy || 'Recruiter'} • {formatDateTime(comment.createdAt || comment.timestamp || '')}
                        ...
                        <p className="text-sm text-gray-500 italic">No notes or comments yet</p>
                    )}
                </div>
    ...
    const renderOfferStage = () => {
        ...
        const sentAt = (offer as ExtendedATSOfferDocument)?.sentAt || (offer as ExtendedATSOfferDocument)?.createdAt
        const acceptedAt = (offer as ExtendedATSOfferDocument)?.signedAt
        const declinedAt = (offer as ExtendedATSOfferDocument)?.declinedAt
        const declineReason = (offer as ExtendedATSOfferDocument)?.declineReason || ''
        
        // Get job details for display
        const jobTitle = atsJob?.title || 'N/A'
        const employmentType = (offer as ExtendedATSOfferDocument)?.employmentType || 'full-time'
        ...
        return (
            <div className="bg-gray-50 rounded-lg p-6 space-y-6">
                ...
                <div className="bg-white rounded-lg p-6 border">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <p className="text-sm text-gray-600">Status</p>
                            <p className="text-sm font-medium text-gray-900">
                                {currentSubStage === OfferSubStage.NOT_SENT ? 'Not Sent' :
                                 currentSubStage === OfferSubStage.OFFER_SENT ? 'Offer Sent' :
                                 currentSubStage === OfferSubStage.OFFER_ACCEPTED ? 'Offer Accepted' :
                                 'Offer Declined'}
                            </p>
                        </div>
                        ...
                        {currentSubStage === OfferSubStage.OFFER_ACCEPTED && acceptedAt && (
                            <div>
                                ...
                                <p className="text-sm font-medium text-gray-900">{formatDateTime(acceptedAt)}</p>
                            </div>
                        )}
                        {currentSubStage === OfferSubStage.OFFER_DECLINED && declinedAt && (
                            <div>
                                <p className="text-sm text-gray-600">Declined On</p>
                                <p className="text-sm font-medium text-gray-900">{formatDateTime(declinedAt)}</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-lg p-6 border space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium text-gray-900">{employmentType}</p>
                            </div>
                            {sentAt && (
                                <div>
                                    <p className="text-sm text-gray-600 mb-1">Sent On</p>
                                    <p className="text-sm font-medium text-gray-900">{formatDateTime(sentAt)}</p>
                                </div>
                            )}
                        </div>
                        {showActions && (
                ...
                {currentSubStage === OfferSubStage.OFFER_ACCEPTED && offer && (
                    <div className="bg-white rounded-lg p-6 border space-y-4">
                        ...
                        <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                            <div className="flex items-center gap-2 mb-2">
                                <CheckCircle className="h-5 w-5 text-green-600" />
                                <span className="text-sm font-medium text-green-900">Offer Accepted by Candidate</span>
                            </div>
                            {acceptedAt && (
                                <p className="text-xs text-green-700">Accepted on {formatDateTime(acceptedAt)}</p>
                            )}
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                ...
                {currentSubStage === OfferSubStage.OFFER_DECLINED && offer && (
                    <div className="bg-white rounded-lg p-6 border space-y-4">
                        ...
                        <div className="p-4 bg-red-50 rounded-lg border border-red-200">
                            <div className="flex items-center gap-2 mb-2">
                                <X className="h-5 w-5 text-red-600" />
                                <span className="text-sm font-medium text-red-900">Offer Declined by Candidate</span>
                            </div>
                            {declinedAt && (
                                <p className="text-xs text-red-700">Declined on {formatDateTime(declinedAt)}</p>
                            )}
                            {declineReason && (
                ...
                {currentSubStage !== OfferSubStage.NOT_SENT && offer && (
                    <div className="bg-white rounded-lg p-6 border">
                        ...
                        <div className="space-y-4">
                            ...
                            {(offer as ExtendedATSOfferDocument).documentUrl && (offer as ExtendedATSOfferDocument).documentFilename ? (
                                <div className="border rounded-lg p-4 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <FileIcon className="h-5 w-5 text-[#4640DE]" />
                                        <div>
                                            <p className="text-sm font-medium text-gray-900">{(offer as ExtendedATSOfferDocument).documentFilename}</p>
                                            <p className="text-xs text-gray-500">Original Offer Letter</p>
                                            {sentAt && (
                                                <p className="text-xs text-gray-500">Sent on {formatDateTime(sentAt)}</p>
                                            )}
                                        </div>
                                    </div>
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => window.open((offer as ExtendedATSOfferDocument).documentUrl, '_blank')}
                                        className="gap-2"
                                    >
                                ...
                                <p className="text-sm text-gray-500 italic">No offer document uploaded</p>
                            )}

                            {/* Signed Document - Show when offer is accepted */}
                            {currentSubStage === OfferSubStage.OFFER_ACCEPTED && (offer as ExtendedATSOfferDocument).signedDocumentUrl && (offer as ExtendedATSOfferDocument).signedDocumentFilename && (
                                <div className="border-2 border-green-200 rounded-lg p-4 flex items-center justify-between bg-green-50">
                                    <div className="flex items-center gap-3">
                                        <CheckCircle className="h-5 w-5 text-green-600" />
                                        <div>
                                            <p className="text-sm font-medium text-gray-900">{(offer as ExtendedATSOfferDocument).signedDocumentFilename}</p>
                                            <p className="text-xs text-green-700 font-medium">Signed Offer Document</p>
                                            {acceptedAt && (
                                                <p className="text-xs text-gray-500">Signed on {formatDateTime(acceptedAt)}</p>
                                            )}
                                        </div>
                                    </div>
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => window.open((offer as ExtendedATSOfferDocument).signedDocumentUrl, '_blank')}
                ...
                {currentSubStage === OfferSubStage.OFFER_SENT && (
                    <div className="bg-white rounded-lg p-6 border">
                        <h4 className="font-semibold text-gray-900 mb-4">Candidate Response</h4>
                        <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                            <div className="flex items-center gap-2">
                                <Clock className="h-5 w-5 text-yellow-600" />
                                <span className="text-sm font-medium text-yellow-900">Waiting for candidate response</span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="bg-white rounded-lg p-6 border">
                    ...
                    {comments.filter(c => c.stage === ATSStage.OFFER).length > 0 ? (
                        <div className="space-y-3">
                            {comments.filter(c => c.stage === ATSStage.OFFER).map((comment, idx) => (
                                <div key={idx} className="border rounded-lg p-4">
                                    <p className="text-sm text-gray-700">{comment.comment}</p>
                                    <p className="text-xs text-gray-500 mt-2">
                                        by {comment.recruiterName || comment.addedByName || comment.addedBy || 'Recruiter'} • {formatDateTime(comment.createdAt || comment.timestamp || '')}
                ...
                {showActions && (
                    <div className="flex flex-col gap-3">
                        {currentSubStage !== OfferSubStage.OFFER_ACCEPTED && currentSubStage !== OfferSubStage.OFFER_DECLINED && (
                            <Button variant="outline" className="gap-2" onClick={() => setShowCommentModal(true)}>
                                <MessageSquare className="h-4 w-4" />
                                Add Comment
                            </Button>
                ...
                {(currentSubStage === OfferSubStage.OFFER_SENT || currentSubStage === OfferSubStage.OFFER_ACCEPTED) && (
    ...
    const candidateName = isATSMode ? (atsApplication?.seeker_name || atsApplication?.name || atsApplication?.full_name || atsApplication?.seeker?.name || 'Candidate') : (candidateData?.user.name || 'Candidate')
    const candidateRole = isATSMode ? (atsApplication?.job?.title || atsApplication?.job?.job_title || atsJob?.title || 'Applicant') : candidateData?.profile.headline
    const candidateEmail = isATSMode ? (atsApplication?.email || atsApplication?.seeker?.email) : candidateData?.user.email
    const candidatePhone = isATSMode ? (atsApplication?.phone || candidateData?.profile?.phone || '-') : (candidateData?.profile.phone)
    const candidateAvatar = isATSMode ? (atsApplication?.seeker_avatar || atsApplication?.seeker?.avatar) : (candidateData?.profile.avatarFileName || candidateData?.profile.avatarUrl)
        ...
        <CompanyLayout>
            <div className="px-6 py-6 space-y-6">
                <div>
                    <Button
                        ...
                        onClick={() => navigate(isATSMode ? '/company/ats' : '/company/candidates')}
                    >
                            ...
                            <CardContent className="p-5">
                                ...
                                <div className="flex items-start gap-4 mb-5">
                                    <Avatar className="w-16 h-16 border-2 border-white shadow-sm">
                                        {candidateAvatar ? (
                                            <AvatarImage src={candidateAvatar} alt={candidateName} className="object-cover" />
                                        ) : null}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h2 className="text-lg font-semibold text-[#25324B] mb-1 truncate" title={candidateName}>{candidateName}</h2>
                                        <p className="text-sm text-[#7C8493] mb-3 truncate" title={candidateRole || undefined}>{candidateRole}</p>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    {candidateScore === -1 || candidateScore === undefined ? (
                                                        ...
                                                        <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                                                            <div
                                                                className={`h-full rounded-full ${candidateScore >= 70 ? 'bg-green-500' : candidateScore >= 40 ? 'bg-yellow-500' : 'bg-red-500'}`}
                                    ...
                                    <>
                                        <div className="bg-[#F8F8FD] rounded-lg p-4 mb-5">
                                            <div className="flex items-center justify-between mb-3">
                                                ...
                                                <span className="text-sm text-[#7C8493]">{getTimeAgo(atsApplication?.applied_date || '')}</span>
                                            </div>
                                            <div className="h-px bg-[#D6DDEB] mb-3"></div>
                                            <div>
                                                <p className="text-sm font-semibold text-[#25324B] mb-1 line-clamp-1" title={atsJob?.title || atsApplication?.job?.title}>
                                                    {atsJob?.title || atsApplication?.job?.title || 'Job Title'}
                                                </p>
                                                <div className="flex items-center gap-2 text-xs text-[#7C8493]">
                                                    <span>{atsJob?.company_name || 'ZeekNet'}</span>
                                                    <span>•</span>
                                                    <span>{atsJob?.employmentTypes?.join(', ') || 'Full-time'}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-[#F8F8FD] rounded-lg p-4 mb-5">
                                            ...
                                            {atsApplication?.stage !== 'APPLIED' && (
                                                <p className="text-xs text-gray-500 mt-2">
                                                    Sub-stage: {atsApplication?.subStage?.replace(/_/g, ' ') || '-'}
                                                </p>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-2 mb-5">
                                            ...
                                            <div className="flex gap-2">
                                                <Button
                                                    ...
                                                    onClick={async () => {
                                                        const seekerId = atsApplication?.seekerId || atsApplication?.seeker_id
                                                        if (!seekerId) return
                                                        try {
                                                ...
                                                {/* Hide reject button when stage is OFFER and substage is OFFER_SENT */}
                                                {!(atsApplication?.stage === ATSStage.OFFER && atsApplication?.subStage === OfferSubStage.OFFER_SENT) && (
                                                    <Button
                                                        variant="outline"
                                                        className="flex-1 border-red-100 text-red-500 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
                                                        onClick={() => {
                                                            if (window.confirm('Reject candidate?')) handleUpdateStage('REJECTED', '')
                                                        }}
                                                    >
                                ...
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        {candidateEmail && (
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <a href={`mailto:${candidateEmail}`} className="text-sm font-medium text-[#25324B] truncate block hover:text-[#4640DE]">{candidateEmail}</a>
                                                </div>
                                            </div>
                                        )}
                                        {candidatePhone && candidatePhone !== '-' && (
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm font-medium text-[#25324B]">{candidatePhone}</p>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {candidateData?.profile?.socialLinks?.map((social, idx) => {
                                            let Icon = Globe
                                            if (social.name.toLowerCase().includes('instagram')) Icon = Instagram
                                            else if (social.name.toLowerCase().includes('twitter')) Icon = Twitter

                                            return (
                                                <div key={idx} className="flex items-start gap-3">
                                                    <Icon className="w-5 h-5 text-[#7C8493] mt-0.5 flex-shrink-0" />
                                                    <div>
                                                        <p className="text-sm text-[#7C8493] mb-0.5 capitalize">{social.name}</p>
                                                        <a href={social.link.startsWith('http') ? social.link : `https://${social.link}`}
                            ...
                            <div className="border-b px-6">
                                <div className="flex gap-8">
                                    <button
                                        ...
                                        className={`py-4 text-sm font-medium border-b-2 transition-colors ${activeTab === 'profile' ? 'border-[#4640DE] text-[#4640DE]' : 'border-transparent text-gray-600 hover:text-gray-900'
                                            }`}
                                    >
                                        Applicant Profile
                                    </button>
                                    <button
                                        ...
                                        className={`py-4 text-sm font-medium border-b-2 transition-colors ${activeTab === 'resume' ? 'border-[#4640DE] text-[#4640DE]' : 'border-transparent text-gray-600 hover:text-gray-900'
                                            }`}
                                    >
                                        Resume
                                    </button>
                                    {isATSMode && (
                                        ...
                                        <button
                                            ...
                                            className={`py-4 text-sm font-medium border-b-2 transition-colors ${activeTab === 'hiring' ? 'border-[#4640DE] text-[#4640DE]' : 'border-transparent text-gray-600 hover:text-gray-900'
                                                }`}
                                        >
                                            Hiring Progress
                                        </button>
                                            <button
                                                ...
                                                className={`py-4 text-sm font-medium border-b-2 transition-colors ${activeTab === 'activity' ? 'border-[#4640DE] text-[#4640DE]' : 'border-transparent text-gray-600 hover:text-gray-900'
                                                    }`}
                                            >
                                                Activity Log
                                            </button>
                                            ...
                                            {hiringStages.map((stage) => {
                                                ...
                                                return (
                                                    <div key={stage.key} className="flex items-center flex-shrink-0">
                                                        ...
                                                        >
                                                            {stage.completed && <CheckCircle2 className="h-4 w-4 inline mr-1" />}
                                                            {stage.current && !stage.completed && <Circle className="h-4 w-4 inline mr-1 fill-current" />}
                                                            {stage.label}
                                                        </button>
                                                    </div>
                                                )
                                                ...
                                                <div>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-6 mb-5">
                                                        <div>
                                                            ...
                                                            <p className="text-sm font-medium text-[#25324B]">
                                                                {candidateData.user.name || candidateData.profile.name || 'N/A'}
                                                            </p>
                                                        </div>
                                                        {candidateData.profile.dateOfBirth && (
                                                            <div>
                                                                ...
                                                                <p className="text-sm font-medium text-[#25324B]">
                                                                    {new Date(candidateData.profile.dateOfBirth).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                                                                </p>
                                                            </div>
                                                        )}
                                                        {candidateData.profile.gender && (
                                                            <div>
                                                                ...
                                                                <p className="text-sm font-medium text-[#25324B] capitalize">
                                                                    {candidateData.profile.gender.charAt(0).toUpperCase() + candidateData.profile.gender.slice(1)}
                                                                </p>
                                                            </div>
                                                        )}
                                                        {candidateData.profile.languages && candidateData.profile.languages.length > 0 && (
                                                            <div>
                                                    ...
                                                    {candidateData.profile.location && (
                                                        <div>
                                                            ...
                                                            <p className="text-sm font-medium text-[#25324B]">{candidateData.profile.location}</p>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    {candidateData.profile.summary && (
                                                        <div className="mb-5">
                                                            ...
                                                            <p className="text-sm text-[#25324B] leading-relaxed">{candidateData.profile.summary}</p>
                                                        </div>
                                                    )}
                                                    {candidateData.profile.skills && candidateData.profile.skills.length > 0 && (
                                                        <div>
                                                            <p className="text-sm text-[#7C8493] mb-2">Skill set</p>
                                                            <div className="flex flex-wrap gap-2">
                                                                {candidateData.profile.skills.map((skill: string, i: number) => (
                                                                    <Badge key={i} variant="secondary" className="bg-gray-100 hover:bg-gray-200 text-gray-700">
                                                                        {skill}
                                                                    </Badge>
                                                                ))}
                                                            </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="flex items-start justify-between mb-6">
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div className="flex-1">
                                                    <h2 className="text-xl font-semibold text-[#25324B] mb-1">{candidateName}</h2>
                                                    <p className="text-sm text-[#7C8493] mb-3">{candidateRole}</p>
                                            ...
                                            {/* View Resume Button */}
                                            {(atsApplication?.resumeUrl || atsApplication?.resume_url || candidateData?.profile?.resume?.url) && (
                                                <Button
                                                    variant="outline"
                                                    className="gap-2"
                                                    onClick={() => window.open(atsApplication?.resumeUrl || atsApplication?.resume_url || candidateData?.profile?.resume?.url, '_blank')}
                                                >
                                                    <Download className="h-4 w-4" />
                                                    View Resume
                                                </Button>
                                        ...
                                        {(atsApplication?.coverLetter || atsApplication?.cover_letter) && (
                                            <div>
                                                <h3 className="text-lg font-semibold text-[#25324B] mb-3">Cover Letter</h3>
                                        ...
                                        {candidateData?.profile?.languages && candidateData.profile.languages.length > 0 && (
                                            <div>
                                                <h3 className="text-lg font-semibold text-[#25324B] mb-3">Languages</h3>
                                                <div className="flex flex-wrap gap-2">
                                                    {candidateData.profile.languages.map((lang: string, i: number) => (
                                                        <span key={i} className="text-sm text-[#25324B]">
                                                            {lang}{i < (candidateData.profile.languages?.length || 0) - 1 ? ', ' : ''}
                                                        </span>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        {candidateData?.experiences && candidateData.experiences.length > 0 && (
                                            <div>
                                                <h3 className="text-lg font-semibold text-[#25324B] mb-3 uppercase">Experience</h3>
                                                <div className="space-y-4">
                                                    {candidateData.experiences.map((exp, i: number) => (
                                                        <div key={i} className="flex gap-4">
                                                            ...
                                                            <div className="flex-1">
                                                                <h4 className="font-medium text-[#25324B]">{exp.title}</h4>
                                                                <p className="text-sm text-[#7C8493]">{exp.company} • {exp.employmentType || 'Full-time'}</p>
                                                                ...
                                                                {exp.location && (
                                                                    <p className="text-sm text-[#7C8493] mt-1">{exp.location}</p>
                                                                )}
                                                                {exp.description && (
                                                                    <p className="text-sm text-[#25324B] mt-2 leading-relaxed">{exp.description}</p>
                                                                )}
                                        ...
                                        {loading ? (
                                            ...
                                            <StageBasedActivityView
                                                ...
                                                onLoadMore={async () => {
                                                    ...
                                                    if (!currentId || !activitiesNextCursor || loadingMoreActivities) return;

                                                    setLoadingMoreActivities(true);
                                                    try {
                                                        const response = await atsService.getActivitiesByApplicationPaginated(
                                                            currentId,
                                                            20,
                                                        ...
                                                        const reversedNewActivities = [...newActivities].reverse();
                                                        setActivities((prev) => {
                                                            // Merge and sort to ensure correct chronological order
                                                            const merged = [...prev, ...reversedNewActivities];
                                                            merged.sort((a, b) => {
                                                        ...
                                                        setActivitiesNextCursor(response.nextCursor || null);
            ...
            {selectedInterviewForFeedback && (
                <InterviewFeedbackModal
                    ...
                    onSubmit={(rating, feedback) => handleSubmitFeedback(selectedInterviewForFeedback.id, rating, feedback)}
                />
            )}
            ...
            {selectedTaskForReview && (
                <InterviewFeedbackModal
                    ...
                    onSubmit={(rating, feedback) => handleSubmitTaskFeedback(selectedTaskForReview.id, rating, feedback)}
                />
            )}
            ...
            {atsApplication && (
                <AssignTaskModal
                    ...
                    taskToEdit={selectedTaskForEdit ? {
                        ...
                        deadline: selectedTaskForEdit.deadline ? new Date(selectedTaskForEdit.deadline).toISOString().split('T')[0] : '',
            ...
            {((atsJob?.enabled_stages && atsJob.enabled_stages.length > 0) || (atsJob?.enabledStages && atsJob.enabledStages.length > 0)) && (
                <MoveToStageModal
                    isOpen={showMoveToStageModal}
                    onClose={() => setShowMoveToStageModal(false)}
                    candidateName={candidateName}
                    currentStage={(selectedStage === 'APPLIED' || selectedStage === 'Applied')
                        ? ATSStage.IN_REVIEW  // APPLIED is UI-only, actual stage is IN_REVIEW
                        : (atsApplication?.stage as ATSStage || ATSStage.IN_REVIEW)}
                    availableStages={(atsJob.enabled_stages || atsJob.enabledStages || []) as ATSStage[]}
            ...
            <CompensationInitModal
                ...
                existingData={compensationData ? {
                    ...
                    notes: compensationNotes.length > 0 ? compensationNotes[0]?.comment || (compensationNotes[0] as { comment?: string; note?: string }).note || '' : undefined
                } : undefined}
            />
            ...
            <CompensationUpdateModal
                ...
                existingNotes={compensationNotes.length > 0 ? (compensationNotes[0]?.comment || (compensationNotes[0] as { comment?: string; note?: string }).note || '') || undefined : undefined}
            ...
            {atsApplication && (
                <CreateOfferModal
                    ...
                    offerToEdit={showEditOfferModal && currentOffer ? {
                        documentUrl: (currentOffer as ExtendedATSOfferDocument).documentUrl || '',
                        documentFilename: (currentOffer as ExtendedATSOfferDocument).documentFilename || '',
                        offerAmount: (currentOffer as ExtendedATSOfferDocument).offerAmount || '',
                        employmentType: (currentOffer as ExtendedATSOfferDocument).employmentType || 'full-time',
                        joiningDate: '',
                        validityDate: '',
                        id: currentOffer.id
                    } : undefined}
                ...
                <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader>
                        ...
                        <DialogDescription className="pt-2">
                            This action should only be used in exceptional cases. Once withdrawn, the offer cannot be reinstated.
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={withdrawReason} onValueChange={setWithdrawReason}>
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="candidate_violated_conditions">Candidate violated conditions</SelectItem>
                                    <SelectItem value="role_cancelled_budget">Role cancelled / budget issue</SelectItem>
                        ...
                        {withdrawReason === 'other' && (
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    className="w-full"
                                />
                                <p className="text-xs text-gray-500">Please provide a detailed explanation for withdrawing this offer.</p>
                            </div>
                        )}
                    </div>
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            onClick={async () => {
                                ...
                                if (withdrawReason === 'other' && !withdrawOtherNote.trim()) {
                                    toast({
                                        title: "Error",
                                        description: "Please provide a note when selecting 'Other' as the reason.",
                                        variant: "destructive"
                                    })
                                    return
                                }
                                ...
                                if (!currentId || !currentOffer?.id) return
                                    ...
                                    try {
                                        ...
                                        const reasonText = withdrawReason === 'other' 
                                            ? withdrawOtherNote 
                                            : withdrawReason.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
                                    ...
                                    toast({
                                        title: "Error",
                                        description: (error as { response?: { data?: { message?: string } } })?.response?.data?.message || "Failed to withdraw offer.",
                                        variant: "destructive"
                                    })
                                } finally {
                            ...
                            disabled={withdrawing || !withdrawReason || (withdrawReason === 'other' && !withdrawOtherNote.trim())}
                        >
